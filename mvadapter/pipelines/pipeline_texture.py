...
from mvadapter.utils.mesh_utils.hy3d_uvwrap_utils import mesh_uv_wrap
...
def mesh_simplify_trimesh(inputpath, outputpath, target_count=40000, save=True):
    """使用 Hunyuan3D 的稳定简化方法"""
    # 先去除离散面
    ...
    # 转为 OBJ
    temp_obj = outputpath.replace(".glb", "_temp.obj")
    ms.save_current_mesh(temp_obj, save_textures=False)

    # 调用减面函数
    current = trimesh.load(temp_obj, force="mesh")
    face_num = current.faces.shape[0]

    print(f"[INFO] Original mesh: {face_num} faces")

    if face_num > target_count:
        print(f"[INFO] Simplifying from {face_num} to {target_count} faces...")
        current = current.simplify_quadric_decimation(face_count=target_count)
        print(f"[INFO] Simplified to {current.faces.shape[0]} faces")
    # # ===== Add validation checks =====
    # # Check for NaN/Inf values
    # if np.any(~np.isfinite(current.vertices)):
    #     print("[WARNING] Found NaN/Inf in vertices, cleaning...")
    #     mask = np.all(np.isfinite(current.vertices), axis=1)
    #     current.update_vertices(mask)
    ...
    if save:
        current.export(outputpath)
        print(f"[INFO] Saved to {outputpath}")
...
def remesh_mesh(mesh_path, remesh_path="remeshed.glb", save=True, target_count=40000):
    """简化网格的入口函数"""
    mesh = mesh_simplify_trimesh(
        mesh_path, remesh_path, save=save, target_count=target_count
    )
    return mesh


def unwrap_with_hy3d(mesh_path: str, save_path: str) -> str:
    mesh = trimesh.load(mesh_path, force="mesh")
    mesh = mesh_uv_wrap(mesh)
    if not isinstance(mesh.visual, TextureVisuals):
        white = np.ones((4, 4, 3), dtype=np.uint8) * 255
        material = SimpleMaterial(image=white)
        mesh.visual = TextureVisuals(uv=mesh.visual.uv, image=white, material=material)
    elif mesh.visual.uv is None:
        mesh.visual.uv = mesh.visual.uv_coords
...
class ModProcessConfig:
    ...
    view_inpaint_min_rounds: int = 8
    view_inpaint_max_rounds: int = 16
...
class TexturePipeline:
    ...
    def __call__(
        ...
        shaded_model_save_path="shaded_model.glb",
        ...
        if uv_unwarp:
            ...
            simplified_path = os.path.join(save_dir, "mesh_simplified.glb")

            # 使用 Hunyuan3D 的简化方法
            remesh_mesh(
                mesh_path, simplified_path, save=True, target_count=face_upper_bound
            )
            mesh_path = simplified_path

            # 执行 UV unwrap
            mesh_path_new = os.path.join(save_dir, "mesh_unwarp.glb")
            print(f"[INFO] Starting UV unwrap...")
            # process_raw(mesh_path, mesh_path_new, preprocess=preprocess_mesh)
            unwrap_with_hy3d(mesh_path, mesh_path_new)
            mesh_path = mesh_path_new
            print(f"[INFO] UV unwrap completed")
        # ==========================================
