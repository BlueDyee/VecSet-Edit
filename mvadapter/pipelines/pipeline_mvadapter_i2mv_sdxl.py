...
class MVAdapterI2MVSDXLPipeline(StableDiffusionXLPipeline, CustomAdapterMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Latents.shape torch.Size([6, 4, 96, 96])
    ...
    def sdedit(
        ...
        **kwargs,
        ...
        mv_image = self.image_processor.preprocess(mv_image)
        latents, noise = self.prepare_image_latents(
                mv_image,
                latent_timestep,
                batch_size,
                num_images_per_prompt,
        ...
        if repaint_mask is not None:
            ...
            # and half precision
            print("🕛 input_mask",repaint_mask.shape)
            print("🕛 target mask", latents.shape[2:])
            init_mask = torch.nn.functional.interpolate(
                repaint_mask, size=latents.shape[2:], mode="nearest"
            )

            init_mask = init_mask.to(device=device, dtype=latents.dtype)
            init_latents = self.prepare_image_latents(
                mv_image,
                latent_timestep,
                batch_size,
                num_images_per_prompt,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if repaint_mask is not None:
                    noised_init_latents = init_latents.clone()

                    if i < len(timesteps) - 1:
                        noise_timestep = timesteps[i + 1]
                        noised_init_latents = self.scheduler.add_noise(
                            noised_init_latents, noise, torch.tensor([noise_timestep])
                        )

                    latents = (1 - init_mask) * noised_init_latents + init_mask * latents
                if latents.dtype != latents_dtype:
                    if torch.backends.mps.is_available():
                        # some platforms (eg. apple mps) misbehave due to a pytorch bug: https://github.com/pytorch/pytorch/pull/99272
                        latents = latents.to(latents_dtype)
