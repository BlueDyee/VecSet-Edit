...
import xatlas
def uv_parameterize_xatlas(mesh):
    """使用 xatlas 进行 UV unwrap（更快）"""
...
### Pack All ###
def process_raw(mesh_path, save_path, preprocess=True, device="cpu", use_xatlas=True):
    ...
    if use_xatlas:
        print("[INFO] Using xatlas for UV unwrap (faster)")
        ...
        t_pos_idx = indices
        normals = normals[vmapping] if normals is not None else None
        
        # xatlas 直接给出每个顶点的 UV，需要转换为 per-face-vertex 格式
        # 展开为 (#F, 3, 2) 然后 reshape 为 (#F*3, 2)
        v_tex_np = uvs  # shape: (#V, 2)
        
        # 创建 per-face-vertex UV 索引（与面索引相同）
        t_tex_idx = t_pos_idx.copy()
        ...
        t_tex_idx = torch.from_numpy(t_tex_idx).to(device=device, dtype=torch.long)
        ...
        print(f"[INFO] Saved UV unwrapped mesh to {save_path}")
        return
