...
def _drop_degenerate_uv_faces(mesh: TexturedMesh) -> None:
    tri = mesh.t_tex_idx
    ...
    print(f"⚠️  Removing {dropped} degenerate UV triangles")
    mesh.t_tex_idx = tri[keep]
    if hasattr(mesh, "t_pos_idx") and mesh.t_pos_idx.shape[0] == tri.shape[0]:
        mesh.t_pos_idx = mesh.t_pos_idx[keep]
    if hasattr(mesh, "t_idx") and mesh.t_idx.shape[0] == tri.shape[0]:
        mesh.t_idx = mesh.t_idx[keep]
...
def uv_precompute(
    ...
    # ===== CRITICAL FIX: Ensure correct dtypes BEFORE validation =====
    # nvdiffrast requires int32 for indices, not int64!
    uv_clip4 = uv_clip4.contiguous().float()
    
    # Convert indices to int32 if they're int64
    if mesh.t_tex_idx.dtype == torch.int64:
        print(f"⚠️  Converting mesh.t_tex_idx from int64 to int32")
        mesh.t_tex_idx = mesh.t_tex_idx.int()
    _drop_degenerate_uv_faces(mesh)
    mesh.t_tex_idx = mesh.t_tex_idx.contiguous()
    # ================================================================
    
    # Check for invalid values
    if torch.any(~torch.isfinite(uv_clip4)):
        print("❌ ERROR: uv_clip4 contains NaN/Inf!")
        uv_clip4 = torch.nan_to_num(uv_clip4, nan=0.0, posinf=1.0, neginf=-1.0)
    
    # Validate indices are within bounds
    if mesh.t_tex_idx.max() >= uv_clip4.shape[0]:
        raise ValueError(f"Index out of bounds: max index {mesh.t_tex_idx.max()} >= vertices {uv_clip4.shape[0]}")
    
    # Clear CUDA cache before heavy operation
