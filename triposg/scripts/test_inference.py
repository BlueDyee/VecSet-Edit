...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-input", type=str, default="./owlcat.png")
    parser.add_argument("--seed", type=int, default=9999999)
    ...
    img_pil = prepare_image(args.image_input, bg_color=np.array([1.0, 1.0, 1.0]), rmbg_net=rmbg_net)


    debug_dict={
        "vis_intermediate":{"interval": 5, "start":25}
    }
    ...
    mesh = trimesh.Trimesh(output_tensor[0].astype(np.float32), np.ascontiguousarray(output_tensor[1]))
    mesh.export(f"output.glb")
    output_dict  = outputs[2]
    for instance in output_dict["vis_intermediate"]:
        step = instance["step"]
        output_tensor = instance["output_tensor"][0]
        mesh = trimesh.Trimesh(output_tensor[0].astype(np.float32), np.ascontiguousarray(output_tensor[1]))
        if args.faces > 0:
            mesh = simplify_mesh(mesh, args.faces)

        mesh.export(f"test_glb/{step}.glb")
