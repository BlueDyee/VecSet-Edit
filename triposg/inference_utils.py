...
def hierarchical_extract_geometry(geometric_func: Callable,
                     ...
                     debug_print=False,
                     debug_print_dict={
                                "sdf_cloud":"./output/sdf_cloud/demo",}
                     ):
    """
    分層提取幾何體：將vec2set生成的SDF網格轉換為三角網格
    
    這個函數實現了多尺度SDF提取和mesh生成的完整流程：
    1. 從低解析度開始生成SDF網格
    2. 逐層增加解析度，只在表面附近計算SDF值（節省計算量）
    3. 使用Marching Cubes算法將最終的SDF網格轉換為三角網格

    Args:
        geometric_func: SDF計算函數（通常是vec2set模型的解碼器）
        device: 計算設備
        bounds: 3D空間邊界
        dense_octree_depth: 初始密集網格深度
        hierarchical_octree_depth: 最終分層網格深度
    Returns:
        包含頂點和面的三角網格列表
    """
    print("🐛 Debug mode (hierarchical_extract_geometry)")if debug_print else None
    # 處理邊界參數：如果是單個數值，轉為6個數值的邊界框
    ...
    # print(f'xyz_samples max: {xyz_samples.max(dim=0).values}, min: {xyz_samples.min(dim=0).values}') if debug_print else None
    
    print(f'step 1 query num: {xyz_samples.shape[0]}')if debug_print else None
    # 步驟2：計算初始網格的SDF值
    # geometric_func是vec2set模型，輸入3D座標，輸出該點的SDF值
    ...
    # 儲存為PLY格式（推薦）
    if debug_print:
        print(f"🎈 grid logits min {torch.min(grid_logits)} max{torch.max(grid_logits)}")
        save_grid_logits_as_pointcloud(
            grid_logits=grid_logits,
            output_path=debug_print_dict["sdf_cloud"],
            format_type="ply",
            threshold=-10,  # logits<threshold
            downsample_factor=2,  # 下採樣減少點數量
            colormap="coolwarm" 
        ) if "sdf_cloud" in debug_print_dict else None
    # print("grid_logits shape = ", grid_logits.shape) if debug_print else None
    # print("grid_logits dtype = ", grid_logits.dtype) if debug_print else None
    # print("grid_logits values grid_logits[0,0,:] = ", grid_logits[0,0,:]) if debug_print else None
    # print("grid_logits values grid_logits[125,125,:] = ", grid_logits[125,125,:]) if debug_print else None
    # 步驟3：分層細化SDF網格
    # 從低解析度逐步提升到高解析度，但只在物體表面附近計算新的SDF值
    ...
    # 步驟7：使用Marching Cubes算法將SDF網格轉換為三角網格
    mesh_v_f = []
    try:
        print("final grids shape = ", grid_logits.shape)if debug_print else None
        # Marching Cubes：在SDF=0的等值面提取三角網格
        # 這是將隱式表面（SDF）轉換為顯式表面（三角網格）的標準算法
...
@torch.no_grad()
def sample_extract_logits(geometric_func: Callable,
                     device: torch.device,
                     xyz_samples,
                     ):

    print(f'step 1 query num: {xyz_samples.shape[0]}')if debug_print else None
    grid_logits = geometric_func(xyz_samples.unsqueeze(0)).to(torch.float16)
...
def flash_extract_geometry(
    ...
    **kwargs,
    ...
    try:
        ...
        print("🤡Error occur during flash extract geometry (mybe set use_flash_decoder=False to use hierachical extract):")
        print(e)
        torch.cuda.empty_cache()
        mesh_v_f = (None, None)
...
def visualize_sdf_save_html(points, normals, sdf_values, output_path="sdf_visualization.html"):
    fig = go.Figure()
    ...
    fig.add_trace(go.Scatter3d(
        ...
        marker=dict(
            ...
            color=sdf_values,
            colorscale='RdBu',
            colorbar=dict(title="SDF Values")
        ),
        name='SDF Points'
    ))
    
    # 添加法向量箭頭 (取樣顯示)
    sample_indices = np.arange(0, len(points), 50)
    ...
    fig.update_layout(
        title="SDF Visualization with Normals",
